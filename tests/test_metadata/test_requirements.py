...
def test_authors(metadata):
    ...
    assert metadata["author"] == "Elena Vollmer, Julian Ruck"
...
def test_emails(metadata):
    ...
    print("email_author is: ", email_author)
    print("email_values is: ", email_values)

    assert email_author == "Elena Vollmer, Julian Ruck"

    assert email_values == "elena.vollmer@kit.edu, "


def test_description(metadata):
    """Tests that metadata provides description information."""
    assert "description" in metadata
    assert ("Thermal Anomaly Segmenter (TASeg)"
            in metadata["description"])
    assert ("leak detection in district heating systems"
            in metadata["description"])
